...
const main = (r, $) => {
  ...
  shape = shape.cut( // sensor hole
    makeBaseBox(22, 22, $.fullH + $.glassH)
      .translate(0, -$.axisDist / 2, -$.bearingH + 1)
      .chamfer(3)
  )
  ...
  shape = [[1, 1],[-1, 1],[-1, -1],[1, -1]].reduce((res, [x, y]) =>
    res.cut(
      makeCylinder(2, 10, [x * $.skrewX, y * $.skrewY + $.axisDist / 2, -7.4])
        .chamfer(1.5)
    ), shape
  )
  ...
  shape = [-1, 1].reduce((res, x) =>
    res.cut(
      makeBaseBox($.connBlackW, $.connBlackH, 10)
        .translate(x * $.connBlackX, $.axisDist / 2)
    ), shape
  )
  ...
  return [
    {name: 'vernier-v0.1.0', shape, color: '#555', opacity: 1}
  ];
};
